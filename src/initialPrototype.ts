...
const playback = document.querySelector('.playback') as HTMLAudioElement; //needed cast to avoid error
const apiUrl = 'http://localhost:8080';
const submit_btn = document.querySelector('#submit') as HTMLButtonElement;
const userMessage = document.querySelector('.userMessage') as HTMLElement;

if (!mic_btn) throw new Error('mic_btn element not found');
if (!submit_btn) throw new Error('submit_btn element not found');
if (!playback) throw new Error('playback element not found');
if (!userMessage) throw new Error('user Message element not found');
...
let recorder:MediaRecorder | null = null;//your microphone
...
async function submitToDB(){
    if(submit_btn.classList.contains("isActive")){
        ...
        let pID = document.querySelector('#pID') as HTMLInputElement;
        if (!pID) throw new Error('pID element not found');
...
async function blobToBytea(blob : Blob) {
